...
class BloxorzSuite extends FunSuite {
  ...
  test("test if done when in goal and not done if in starting position") {
    new Level1 {
      assert(done(Block(Pos(4,7),Pos(4,7))) == true)
      assert(done(Block(Pos(1,1),Pos(1,1))) == false)
    }
  }
  ...
  test("test if neighbors with history is determined correctly") {
    new Level1 {
